...
from PCG.Generator import generator
...
with open(join('data/shapenet/splits/', 'train_models.json'), 'r') as f:
    train_models_dict = json.load(f)

with open(join('data/shapenet/splits/', 'val_models.json'), 'r') as f:
...
for epoch in tqdm(range(opt.nepoch+1)):
